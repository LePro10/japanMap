...
export interface VehicleSpec {
  ...
  /** Packed-dirt lateral factor, 0,65…0,85 — ASTRA_PLAN §6. */
  readonly dirt: number;
  /** Fording depth, metres. Ocean beyond this returns the car to shore. */
  readonly ford: number;
  /** Static ground clearance, metres (`collision.band[0]`). */
...
interface Identity {
 ...
 layout: DriveLayout; grip: number; dirt: number; clearance: number; ford: number; blurb: string;
}
function identity(v: Identity): VehicleSpec {
 ...
 return makeSpec({ id:v.id, name:v.name, category:v.category, price:v.price, blurb:v.blurb,
  dirt:v.dirt, ford:v.ford, clearance:v.clearance, chassis,
