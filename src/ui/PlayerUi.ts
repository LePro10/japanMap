...
export class PlayerUi {
  ...
  #syncMapDock(): void {
    const host = this.#menu.querySelector<HTMLElement>("[data-map-dock]");
    const fallback = this.#menu.querySelector<HTMLElement>(".menu__mapFallback");
      ...
      <section class="menu__panel menu__panel--map" data-panel="map" hidden>
        <div class="menu__mapDock" data-map-dock>
          <div class="menu__mapFallback">
            <p class="menu__eyebrow">TAKE A DIFFERENT TURN</p>
            <h1>Island Atlas</h1>
            <div class="menu__mapHero"><img src="${aerialMapUrl}" alt="Aerial map of the island" loading="lazy" /></div>
            <p class="menu__intro">Trace the pass, set a waypoint, or follow the coast.</p>
            <button class="menu__openMap">Open map</button>
          </div>
        </div>
      </section>
